...
const[memeFile1,setMemeFile1] = useState([]);
  ...
  const getUser = async () => {
    try {
      const response = await axios.get(`${newApiUrl}/vendors_details.php?id=edit&vendor_id=${vendor_id}`)
...
const handleUpdate = (event) => {
   ...
   axios.put(`${newApiUrl}/vendor_details.php?id=update&vendor_id=${vendor_id}`, vendor).then(function(response){
